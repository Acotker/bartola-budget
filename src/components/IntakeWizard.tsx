...
// ── Form row types (dollars as strings while typing) ─────────────────────────
interface AssetRow { label: string; dollars: string; spendable: boolean }
...
function StepAssets({
  ...
  return (
    <div className="space-y-5">
      <p className="text-ink/70 text-sm leading-6">
        Any other accounts? Mark anything you won&apos;t spend these two years —
        an emergency fund, retirement — as untouchable, so it doesn&apos;t inflate
        your number.
      </p>
      {assets.map((a, i) => (
        <div key={i} className="bg-card border-line space-y-3 rounded-2xl border p-4">
          <input
            ...
            placeholder="Account name (e.g. HYSA)"
